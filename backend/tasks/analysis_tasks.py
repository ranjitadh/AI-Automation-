...
from apps.analysis.models import JobAnalysis, ATSAnalysis
from apps.analysis.services import analyze_job_fit, analyze_ats
...
def analyze_ats_task(job_id, resume_id):
    try:
        job = Job.objects.get(id=job_id)
    except ObjectDoesNotExist:
        logger.error(f"Job {job_id} not found")
